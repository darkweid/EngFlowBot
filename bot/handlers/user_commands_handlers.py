...
from bot.lexicon import BasicButtons, MainMenuButtons, MessageTexts
...
@user_commands_router.message(
    Command(commands=["start"]), StateFilter(default_state)
)  # default state, user is not registered yet
async def process_start_command(
    ...
    await message.answer(
        MessageTexts.WELCOME_NEW_USER.format(
            user_name=full_name,
        ...
        reply_markup=await keyboard_builder(
            1, set_tz_new_user=BasicButtons.TURN_ON_REMINDER
        ),
    )
    await message.answer(
...
@user_commands_router.message(
    Command(commands=["main_menu"]), ~StateFilter(default_state)
)  # user already registered
@user_commands_router.message(Command(commands=["start"]), ~StateFilter(default_state))
async def process_start_command_existing_user(
    message: Message,
    state: FSMContext,
    user_service: UserService,
):
...
async def info_command(
    ...
    await state.set_state(UserFSM.default)
    await message.answer(
        MessageTexts.INFO_RULES.format(owner_tg_link=settings.owner_tg_link),
        link_preview_options=LinkPreviewOptions(is_disabled=True),
