...
from bot.lexicon import BasicButtons, MessageTexts
from bot.services.user import UserService
from bot.states import UserFSM
from bot.utils import schedule_reminders, time_zones

user_reminder_router: Router = Router()


@user_reminder_router.message(
    Command(commands=["reminder"]), ~StateFilter(default_state)
)
...
\nЕсли нужно - можешь изменить часовой пояс или установить время напоминаний""",
        ...
        await message.answer(
            """<b>Часовой пояс не установлен\nНапоминания выключены</b>
\nМожешь установить свой часовой пояс и затем время напоминаний""",
            reply_markup=await keyboard_builder(
                1, BasicButtons.CHANGE_TIME_ZONE, BasicButtons.CLOSE
            ),
        )
...
async def choose_timezone(
    ...
    await callback.message.answer(
        MessageTexts.CHOOSE_TIMEZONE,
        reply_markup=await keyboard_builder(
            4, BasicButtons.CLOSE, args_go_first=False, **time_zones
        ),
    )
...
async def set_timezone(
    ...
    await callback.message.edit_text(
        f"""Установлен часовой пояс {time_zones.get(callback.data)}
Теперь ты можешь установить время напоминаний""",
        reply_markup=await keyboard_builder(
            1, BasicButtons.CHANGE_REMINDER_TIME, BasicButtons.CLOSE
        ),
    )
    await user_service.set_timezone(
        user_id=callback.from_user.id, timezone=callback.data.split("|")[1]
    )
...
async def set_reminder(
    ...
    await callback.answer()
    await callback.message.answer(
        """В какое время тебе напоминать заниматься?
...
async def set_reminder_time(
    ...
    try:
        ...
        await message.answer(
            f'Отлично, буду напоминать тебе заниматься каждый день в {time.strftime("%H:%M")}'
        )
    except ValueError:
        await message.delete()
