...
from bot.keyboards import keyboard_builder
...
from bot.utils import send_message_to_admin, update_state_data
...
@user_testing_router.callback_query(
    F.data == MainMenuButtons.TESTING
)  # choose section for testing
async def start_testing_with_rules(
    ...
    await callback.answer()
    await callback.message.edit_text(
        MessageTexts.TESTING_HELLO,
        reply_markup=await keyboard_builder(
            1, rules_testing=BasicButtons.RULES, close_rules_tests=BasicButtons.CLOSE
        ),
    )
    # await asyncio.sleep(3)
    sections = await testing_service.get_section_names()
    await callback.message.answer(
        MessageTexts.CHOOSE_SECTION,
        reply_markup=await keyboard_builder(
            1, *[section for section in sections], BasicButtons.MAIN_MENU
        ),
    )
...
@user_testing_router.callback_query(
    (F.data == BasicButtons.BACK), StateFilter(TestingFSM.selecting_subsection)
)
...
)  # choose subsection for testing
async def choosing_section_testing(
    callback: CallbackQuery,
    state: FSMContext,
    testing_service: TestingService,
):
...
)  # subsection chosen, got in callback
async def choosing_subsection_testing(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"""<b>«{subsection}»</b>{additional_rules}
Are you ready?""",
        reply_markup=await keyboard_builder(
            1,
            BasicButtons.MAIN_MENU,
...
async def chose_subsection_testing(
    callback: CallbackQuery,
    state: FSMContext,
    testing_service: TestingService,
    ...
    subsection, section, user_id = (
        data.get("subsection"),
        data.get("section"),
        callback.from_user.id,
    )
    exercise = await testing_service.get_random_testing_exercise(
    ...
    if exercise:
        ...
        first_try_count, success_count, total_exercises_count = (
            await user_progress_service.get_counts_completed_exercises_testing(
                user_id=user_id, section=section, subsection=subsection
            )
        )
        await callback.message.answer(
            f"""{MessageTexts.ALL_EXERCISES_COMPLETED}
...
С первой попытки: <b>{first_try_count}</b>""",
    ...
    await update_state_data(
        state, current_test=test, current_answer=answer.strip(), current_id=id_exercise
    )


@user_testing_router.message(StateFilter(TestingFSM.in_process))  # in testing process
async def in_process_testing(
    message: Message,
    state: FSMContext,
    testing_service: TestingService,
    user_progress_service: UserProgressService,
    ...
    data = await state.get_data()
    section, subsection, exercise_id, user_id = (
        data.get("section"),
        data.get("subsection"),
        data.get("current_id"),
    ...
    if message.text.lower().replace(" ", "") == answer.lower().replace(" ", ""):
        first_try = await user_progress_service.mark_exercise_completed(
...
С первой попытки: <b>{first_try_count}</b>""",
            ...
            await send_message_to_admin(
                f"""Пользователь @{username} выполнил тест
<b>{section} – {subsection}</b>\nС первой попытки <b>{first_try_count} из {success_count}</b>"""
        ...
        await message.answer(
            ...
            reply_markup=await keyboard_builder(
                1, see_answer_testing=BasicButtons.SEE_ANSWER
            ),
        )
        await user_progress_service.mark_exercise_completed(
            exercise_type="Testing",
            section=section,
            subsection=subsection,
            exercise_id=exercise_id,
...
async def start_again_testing(callback: CallbackQuery):
    await callback.answer()
    await callback.message.edit_text(
        MessageTexts.ARE_YOU_SURE_START_AGAIN,
        reply_markup=await keyboard_builder(
            1,
            BasicButtons.CLOSE,
            args_go_first=False,
...
async def start_again_testing(
    ...
    await user_progress_service.delete_progress_by_subsection(
        user_id=user_id, section=section, subsection=subsection
    )
    await callback.message.edit_text(
        f"Прогресс по тесту {section} – {subsection} сброшен"
    )
